...
class Window():
    def __init__(self, title='汽车下车体方案数据库', width=300, height=120, staFunc=bool, stoFunc=bool):
        self.w = width
        self.h = height
        self.stat = True
    ...
    # 用于将窗口居中  
    def center(self):
        ws = self.root.winfo_screenwidth()      #获取屏幕的宽度
        hs = self.root.winfo_screenheight()     #获取屏幕的长度
        x = int( (ws/2) - (self.w/2) )
        y = int( (hs/2) - (self.h/2) )
